...
%% --------------------------------------------------------------------
%% External exports
-export([receive_headers/2,receive_ssl_headers/2,start_link/0]).

%% gen_server callbacks
-export([init/1, handle_call/3, handle_cast/2, handle_info/2, terminate/2, code_change/3]).

-record(state, {sock,request,headers,recv_buff,requestor,sock_owner,csock_mod}).
...
receive_headers(Pid,Sock) ->
	...
	gen_server:call(Pid,{receive_headers,Sock,self(),gen_tcp},600000).

receive_ssl_headers(Pid,Sock) ->
	ssl:controlling_process(Sock,Pid),
	gen_server:call(Pid,{receive_headers,Sock,self(),ssl},600000).
...
handle_call({receive_headers,Sock,SockOwner,SockMod},From,State) when State#state.requestor == undefined ->
	Mod = case SockMod of
			  ssl -> ssl;
			  gen_tcp -> inet
		  end,
	Mod:setopts(Sock,[{active,once}]),
	{noreply,State#state{sock=Sock,requestor=From,sock_owner=SockOwner,csock_mod=SockMod}};
...
%% 	io:format("~p recv_buff: ~p~n",[?MODULE,State#state.recv_buff]),
	case proxylib:find_binary_pattern(State#state.recv_buff,<<"\r\n\r\n">>) of
		nomatch ->
			Mod = case State#state.csock_mod of
					  ssl -> ssl;
					  gen_tcp -> inet
				  end,
			Mod:setopts(State#state.sock,[{active,once}]),
			{noreply,State};
		Idx ->
			<<Hdr:Idx/binary,"\r\n\r\n",Body/binary>> = State#state.recv_buff,
			{ok,Request,HdrList} = proxylib:split_headers(binary_to_list(Hdr)),
			(State#state.csock_mod):controlling_process(State#state.sock,State#state.sock_owner),
			Re = #proxy_pass{headers=HdrList,recv_buff=Body,request=Request,csock_mod=State#state.csock_mod},
...
handle_info({ssl,_Sock,<<Msg/binary>>},State) ->
	gen_server:cast(self(),parse_headers),
	<<RecvBuff/binary>> = State#state.recv_buff,
	{noreply, State#state{recv_buff = <<RecvBuff/binary,Msg/binary>>}};
